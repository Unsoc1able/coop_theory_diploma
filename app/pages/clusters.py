...
Если при импорте страницы появляется ошибка про ``dataclass`` и ``class Threshold``,
значит в окружении осталась старая версия файла (например, установленная как пакет
или закэшированная ``.pyc``). Удалите старые артефакты и убедитесь, что
используется актуальная версия модуля без dataclass.
"""
